import { LessonContent, LessonType, LessonLevel } from '@/types';

const VOCABULARY_TEMPLATES: Record<string, Partial<LessonContent>> = {
  Education: {
    whatYouWillLearn: [
    ...
    commonMistakes: [
      ...
      { mistake: 'The education system needs to change.', correction: 'The education system needs to undergo reform.', explanation: '"Undergo reform" sounds more natural in academic writing than "change".' }
    ],
    collocations: ['academic achievement', 'acquire knowledge', 'implement policies', 'foster development', 'demonstrate proficiency', 'undergo reform', 'enhance engagement', 'critical thinking skills'],
    synonyms: [
  ...
  Technology: {
    ...
    collocations: ['digital transformation', 'technological innovation', 'artificial intelligence', 'digital divide', 'privacy concerns', 'cybersecurity threats', 'global connectivity'],
    synonyms: [
  ...
  Environment: {
    whatYouWillLearn: [
      'Master 15 advanced environmental terms for Band 7+ writing',
    ...
    collocations: ['carbon emissions', 'biodiversity loss', 'sustainable development', 'renewable energy', 'carbon footprint', 'environmental degradation', 'climate change mitigation', 'ecological balance'],
    synonyms: [
...
const GRAMMAR_TEMPLATES: Record<string, Partial<LessonContent>> = {
  Conditionals: {
    whatYouWillLearn: [
...
function generateMiniPractice(type: LessonType, topic: string): LessonContent['miniPractice'] {
  if (type === 'vocabulary') {
    return [
      { question: `The government _____ (implement) several reforms to improve the ${topic.toLowerCase()} sector.`, type: 'fill-blank' },
      { question: 'Which word best completes the sentence?', options: ['acquire', 'make', 'do', 'have'], type: 'multiple-choice' },
      { question: `Rewrite using more academic vocabulary: "The ${topic.toLowerCase()} situation is getting worse."`, type: 'rewrite' },
      { question: `The _____ of new technologies has transformed the ${topic.toLowerCase()} landscape.`, type: 'fill-blank' },
      { question: 'Which collocation is most natural?', options: ['significant impact', 'big impact', 'large impact', 'huge impact'], type: 'multiple-choice' }
    ];
  } else {
    return [
      { question: 'If the government _____ (invest) more in this area, results would improve.', type: 'fill-blank' },
      { question: 'Which sentence is grammatically correct?', options: ['If I was you, I would reconsider.', 'If I were you, I would reconsider.', 'If I would be you, I would reconsider.', 'If I am you, I would reconsider.'], type: 'multiple-choice' },
      { question: 'Rewrite using a more formal structure: "If I had known, I would have helped."', type: 'rewrite' },
      { question: 'By next year, the project _____ (complete).', type: 'fill-blank' },
      { question: 'Which passive construction is correct?', options: ['It is believed that...', 'It believes that...', 'It is believing that...', 'It was believe that...'], type: 'multiple-choice' }
    ];
  }
}

function generateAnswerKey(type: LessonType): string[] {
  if (type === 'vocabulary') {
    return [
      'has implemented',
      'acquire',
      'The situation is deteriorating / The circumstances are worsening significantly.',
      'proliferation / emergence / advent',
      'significant impact'
    ];
  } else {
    return [
...
export async function generateLessonWithAI(
  ...
  const bandRange = level === 'beginner' ? 'Band 5.0 - 6.0' : level === 'intermediate' ? 'Band 6.5 - 7.5' : 'Band 7.5 - 9.0';
  
  let template: Partial<LessonContent>;
  
  if (type === 'vocabulary') {
    template = VOCABULARY_TEMPLATES[topic] || VOCABULARY_TEMPLATES['Education'];
  } else {
    template = GRAMMAR_TEMPLATES[topic] || GRAMMAR_TEMPLATES['Conditionals'];
  }

  const title = type === 'vocabulary' 
    ? `${topic} Vocabulary for IELTS ${bandRange}`
    : `Mastering ${topic} for IELTS ${bandRange}`;

  const coreExplanation = type === 'vocabulary'
    ? `${topic} vocabulary is crucial for IELTS as it appears frequently in Reading passages, Writing Task 2 topics, and Speaking Part 3 discussions. This lesson focuses on words that demonstrate academic sophistication while remaining natural in context.

The key to using academic vocabulary effectively is understanding not just the meaning, but also the collocations (words that naturally go together) and the register (formal vs informal usage). ${bandRange} candidates consistently use these words accurately and appropriately.

For ${level} level learners, we focus on ${level === 'beginner' ? 'foundational academic words that are essential for basic communication' : level === 'intermediate' ? 'high-frequency academic words that show good vocabulary range' : 'sophisticated, nuanced vocabulary that demonstrates mastery'}.`
    : template.coreExplanation || `This grammar structure is essential for achieving ${bandRange} in IELTS. Understanding when and how to use it correctly will significantly improve your writing and speaking scores.`;

  const quickRecap = type === 'vocabulary'
    ? `Today you learned key ${topic.toLowerCase()} vocabulary for ${bandRange}. Remember to use these words with their correct collocations and in appropriate contexts. Practice using them in your next Writing Task 2 or Speaking response!`
    : `Remember the key rules for ${topic.toLowerCase()}: form, meaning, and common errors to avoid. Practice using these structures in your writing and speaking to demonstrate grammatical range and accuracy.`;

  return {
    title,
    targetLevel: bandRange,
    whatYouWillLearn: template.whatYouWillLearn || [
      `Master key ${type} related to ${topic}`,
      `Use these ${type === 'vocabulary' ? 'words' : 'structures'} accurately in IELTS`,
      `Avoid common mistakes that lower your band score`
    ],
    coreExplanation,
    examples: template.examples || [],
    commonMistakes: template.commonMistakes || [],
    miniPractice: generateMiniPractice(type, topic),
    answerKey: generateAnswerKey(type),
    quickRecap,
    ...(type === 'vocabulary' ? {
      collocations: template.collocations || [],
      synonyms: template.synonyms || [],
      speakingLines: template.speakingLines || []
    } : {
      grammarForm: template.grammarForm || '',
      grammarUse: template.grammarUse || '',
      sentenceUpgrade: template.sentenceUpgrade || []
    })
  };
}
