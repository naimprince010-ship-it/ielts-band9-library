import { ArrowRight, CheckCircle, Lightbulb, PencilRuler, SearchCheck, ShieldCheck, Wand2 } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';

interface GrammarOverviewCardProps {
  targetLevel: string;
  whatYouWillLearn: string[];
}

/**
 * "What You Will Learn" section for the grammar workspace template.
 * Always rendered — `whatYouWillLearn` and `targetLevel` are required
 * fields on LessonContent.
 */
export function GrammarOverviewCard({ targetLevel, whatYouWillLearn }: GrammarOverviewCardProps) {
  const pathSteps = [
    { label: 'Diagnose', hint: 'Spot the Band 7 grammar gap', icon: SearchCheck, tone: 'from-blue-500 to-indigo-500' },
    { label: 'Control', hint: 'Choose the right structure', icon: ShieldCheck, tone: 'from-indigo-500 to-violet-500' },
    { label: 'Upgrade', hint: 'Rewrite with precision', icon: Wand2, tone: 'from-violet-500 to-fuchsia-500' },
    { label: 'Proofread', hint: 'Remove tiny errors', icon: PencilRuler, tone: 'from-emerald-500 to-teal-500' },
  ];
