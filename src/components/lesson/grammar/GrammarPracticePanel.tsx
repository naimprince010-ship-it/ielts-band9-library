...
interface GrammarPracticePanelProps {
...
/**
 * "Lesson resources" accordion: Common Mistakes, Mini Practice
 * (InteractivePractice, wired to the matching answerKey entries by index,
 * same as the original LessonPage.tsx behavior), and the Quick Recap
 * card. commonMistakes, miniPractice, answerKey and quickRecap are all
 * required LessonContent fields, so this panel is always rendered.
 */
export function GrammarPracticePanel({ commonMistakes, miniPractice, answerKey, quickRecap }: GrammarPracticePanelProps) {
  return (
        ...
        <AccordionTrigger className="bg-gradient-to-r from-amber-50 via-white to-blue-50 px-5 py-4 text-left hover:no-underline">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm font-normal text-slate-500">Fix the common mistakes, answer the mini practice, then review the Band 8 rule.</p>
            </div>
          </div>
        </AccordionTrigger>
            ...
            <CardContent className="grid gap-4 md:grid-cols-[1fr_260px]">
              ...
              <div className="rounded-2xl border border-white/15 bg-white/10 p-4">
                ...
                <p className="mt-2 text-sm leading-6 text-indigo-100">
                  Do not add complex grammar everywhere. Use one controlled structure when it makes the meaning clearer.
                </p>
              </div>
            </CardContent>
          </Card>
